...
- Or use the [RStudio Addin](https://rstudio.github.io/rstudioaddins/)<sup>1</sup> "Infinite Moon Reader" for a live preview
...
- Icons can be inserted  _inline_ using the package `{icon}`, e.g.`r icon::simple_icons("github")` :  `icon::simple_icons("github")`. And you can retrieve the icons of the following libraries:     
  ...
  - `r icon::fontawesome("user-graduate")` [`academicicons()`](https://jpswalsh.github.io/academicons/): e.g. `doi`, `moodle`, `orcid`, etc.
...
```{r, echo = TRUE}
iris %>% head()  %>% knitr::kable(, format = 'html')
```
